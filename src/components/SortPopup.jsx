...
const SortPopup = React.memo ( function SortPopup( {items, activeSortType, onClickSortType}) {
  ...
  React.useEffect(() => {
     document.body.addEventListener('click', handleOutsideClick);     // Отлавливание клика в ДОМЕ , используеться useEffect для того чтобы отловить его после первого рендера
         ...
         {visiblePopup && <div className="sort__popup">
            <ul>
              {items && items.map((obj, index) => (  // Итерация по массиву с категориями 
                ...
                {obj.name}
                </li>// Более уникальное значение <li> key={`${name}_${index}`}>
             )) }
            </ul>
          </div>}
        </div>)
